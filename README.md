...
- Porta já em uso (ex.: `3000`):
  - Pare o que estiver usando a porta **ou** suba o compose em outra porta.
  - Exemplo (PowerShell): ` $env:FRONTEND_PORT=3001; docker compose up --build `
- Erro “Ollama não está rodando”: verifique se `ollama serve` está ativo e se `OLLAMA_URL`/`OLLAMA_BASE_URL` apontam para o host correto.
- Primeira chamada de RAG pode demorar: embeddings/modelo podem estar carregando e a coleção do Chroma ainda não existe.
